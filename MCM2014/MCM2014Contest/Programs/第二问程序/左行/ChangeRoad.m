...
for i=1:car.car_num
   ...
   if car.road(i)==2 %>>>>>>>>>>>>>>>>> 检车到车在右侧 <<<<<<<<<<<<<<<<<<<<<<<% R->L:  
         ...
         if (Front>car.velocity(i))&&(Back>=dsafe)           %满足换道准则
              ...
              %%%%%%%%%%%%% 向左换道 %%%%%%%%%%%%%%%%%
                 ...
                 new_cells.position(new_car.road(i)+1,new_car.position(i))=false;                  
                 new_cells.position(new_car.road(i),new_car.position(i))=true;             % 元胞更新
         ...
         if (car.velocity(i)>Gap)&&(ChangeFlag==0)&&...
            (Front>Gap)&&(Back>=dsafe)&&(Front>car.velocity(i))      %满足换道准则
