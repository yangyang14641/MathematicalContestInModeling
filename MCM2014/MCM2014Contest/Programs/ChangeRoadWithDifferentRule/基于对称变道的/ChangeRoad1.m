...
%%
 ChangeFrame=0;
 if (vihcle_space.v(roadVihcleNumChain(i,j))>CellFront)&&(CellLeftFront>vihcle_space.v(roadVihcleNumChain(i,j)))&&(CellLeftFront>CellFront)&&(CellLeftBack>dsafe)
 ...
 else if (ChangeFrame==0)&&(vihcle_space.v(roadVihcleNumChain(i,j))>CellFront)&&(CellRightFront>vihcle_space.v(roadVihcleNumChain(i,j)))&&(CellRightFront>CellFront)&&(CellRightBack>dsafe)
      rand('state',sum(100*clock)*rand(1));%定义随机种子
      p_rand=rand;%产生随机概率
