%3,换道模型  (基于靠右行的)
 ...
 %% 进行变道
 ChangeFrame=0;
 if (vihcle_space.v(roadVihcleNumChain(i,j))>CellFront)...
         &&(CellLeftFront>vihcle_space.v(roadVihcleNumChain(i,j)))...
         &&(CellLeftFront>CellFront)&&(CellLeftBack>=dsafe)         %检测向左变道刺激是否满足
 ...
 else if (ChangeFrame==0)&&(CellRightFront>vihcle_space.v(roadVihcleNumChain(i,j)))&&(CellRightBack>=dsafe)
      rand('state',sum(100*clock)*rand(1));%定义随机种子
      p_rand=rand;%产生随机概率
