...
datam_Frenquency_R=zeros(m,n);
datam_Velocity_R=zeros(m,n); 

for i=1:m
    for j=1:n
        datam_Frenquency_R(i,j) = sum(data_matrix_Frenquency(i,j,2:k));
...
VR=zeros(m,1);
for i=1:m
        FR(i)=mean(datam_Frenquency_R(i,:)); 
...
ylabel('change road times');
legend('Keep Left','Symmetry','Keep Right')
