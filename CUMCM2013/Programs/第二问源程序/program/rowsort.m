%%%%%%%%%%%%%%%%%%%%    将根据行的聚类结果拼接图像   %%%%%%%%%%%%%%%%%%%%%%%
function output=rowsort(colnumvector,cutoff)
%% 数据导入
load data1;     %%中文图象数据
%load data2;    %%英文图象数据
...
%% 图像类型转化
for j=1:len
         Imgcellmatrix{j}=double(Imgcellmatrix{j});
         Imgcellmatrix{j}=255-Imgcellmatrix{j};
end
%% 选出根据行间距被聚为一类的图
Img=cell(1,len1);
for j=1:len1
      Img(j)=Imgcellmatrix(colnumvector(j));
end
%% 图像的边界左右各一条边界
for i=1:2
    for j=1:len1
