...
qm3=4*698;    % 草街4台机组
qm4=6*500;    % 井口4台机组
% 提高各电站的发电流量从而调高发电机功率 第三问备用
%qm1=2*200*(1+0.05);    %发电机流量各提高5%
%qm2=4*388.9*(1+0.05);
%qm3=4*698*(1+0.05);
%qm4=6*500*(1+0.05);
% 边界条件水库初始库容
     ...
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%梯级电站的总体年发电量
      ...
      for i=1:4:48   % 水轮机水头上界约束
          ...
          G1(i+3)=max([H(i+3)-8.04,0]);
           %第三问备用
          %G1(i)=  max([H(i)-3.4*(1+0.05),0]);   %水头上限也各放宽5%
          %G1(i+1)=max([H(i+1)-8.8*(1+0.05),0]);
          %G1(i+2)=max([H(i+2)-26.7*(1+0.05),0]);
          %G1(i+3)=max([H(i+3)-8.04*(1+0.05),0]);
      end
